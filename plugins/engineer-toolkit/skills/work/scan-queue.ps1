...
.PARAMETER Workspace
  Workspace root containing the Active\ folder. Default: C:\ClaudeWorkspace.

.PARAMETER VpnHost
  On-prem host to probe for VPN connectivity. Default: tfs.preferredcredit.net.
...
.EXAMPLE
  powershell -NoProfile -File scan-queue.ps1
.EXAMPLE
  powershell -NoProfile -File scan-queue.ps1 -Target CO-322
.EXAMPLE
  powershell -NoProfile -File scan-queue.ps1 -Workspace D:\Work -VpnHost git.corp.local
#>
[CmdletBinding()]
param(
    [string]$Workspace = 'C:\ClaudeWorkspace',
    [string]$VpnHost   = 'tfs.preferredcredit.net',
    [string]$Target,
    [switch]$SkipVpn
)
