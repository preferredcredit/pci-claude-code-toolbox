...
2. **Determine action by `Status:` and `Tier:`:**
   ...
   | Any status the doctrine marks non-actionable | — | Per the *Local `Status:` values* table in workflow.md (the authority): statuses whose `/work` action is "None" — `Code Review`, `Development Complete` — plus `Complete` (swept by `/status`) and the parity statuses `QA` / `QA Complete` / `Deployed`. Skip with warning logged for the summary: `[<KEY>] Skipped — Status=<status> is not actionable by /work.` |
