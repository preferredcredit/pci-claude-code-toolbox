...
**Run it with the PowerShell tool, not Bash** (Bash eats Windows-path backslashes). Invoke:
...
- **Targeted** — focused session in this chat. Execute via `superpowers:subagent-driven-development` (review between tasks, in-chat). Plan review and the triage `< 80` confirm are **chat turns**, not `go` edits — you already cleared `go` on claim and you never re-add it to progress; you just continue the conversation. See [references/triage.md](../../references/triage.md) for the per-mode confidence behavior.
