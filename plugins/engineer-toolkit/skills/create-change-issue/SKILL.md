...
For a typical SOFTDEV deploy CAB:
...
If you're editing an existing ticket via `editJiraIssue`, the same shape works under `fields:` instead of `additional_fields:`. Always pass `contentFormat: "adf"`.
...
- Code (inline): `{"type":"text","text":"...","marks":[{"type":"code"}]}`
- Bullet list: `{"type":"bulletList","content":[{"type":"listItem","content":[{"type":"paragraph","content":[{"type":"text","text":"item"}]}]}]}`

Don't use markdown headings (`##`) inside text nodes — they render literally. Use heading nodes.
